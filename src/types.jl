...
# Back-compat: the pre-header five-field positional shape still constructs.
# GENERIC (untyped) like the released autogenerated constructor, whose
# converting method accepted convertible argument types
ToolParameter(name, description, type, required, default) =
    ToolParameter(name, description, type, required, default, nothing)
...
# GENERIC (untyped) like the released autogenerated constructor, whose converting
# method accepted e.g. `[]` for the vectors and SubString names — an exactly-typed
# forwarder would restore only half the released surface
MCPPrompt(name, description, arguments, messages, title, icons, _meta) =
...
# GENERIC (untyped) like the released autogenerated constructor (see the
# MCPPrompt shim above for why)
ResourceTemplate(name, uri_template, mime_type, description, title, icons,
                 data_provider, _meta) =
    ResourceTemplate(name, uri_template, mime_type, description, title, icons,
                     data_provider, _meta, nothing)
