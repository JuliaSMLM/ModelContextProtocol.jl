...
mutable struct Server
    ...
    legacy_state::Union{ServerState,Nothing}  # the running loop's session state (set by start!, retired on loop exit) — how notify_* reach the legacy session's wire_subscriptions
    subscriptions_lock::ReentrantLock  # guards `subscriptions` (subscribe!/unsubscribe! vs notify_resource_updated snapshots, cross-task)
